...
switch($_GET["op"]) {
    ...
    case "listar":
        ...
        foreach($datos as $row){
            ...
            $sub_array[] = '<button type="button" onClick="editar('.$row["PROV_ID"].')" id="'.$row["PROV_ID"].'" class="btn btn-soft-warning btn-icon waves-effect waves-light layout-rightside-btn"><i class="ri-edit-line label-icon align-middle"></i> </button>';
            $sub_array[] = '<button type="button" onClick="eliminar('.$row["PROV_ID"].')" id="'.$row["PROV_ID"].'" class="btn btn-soft-danger btn-icon waves-effect waves-light layout-rightside-btn"><i class="ri-delete-bin-5-line label-icon"></i> </button>'; 
            $data[] = $sub_array;
        }
